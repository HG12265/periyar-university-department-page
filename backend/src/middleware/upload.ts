...
export const processUploadedFile = async (req: Request, res: Response, next: NextFunction) => {
  try {
    if (!req.file) {
    ...
    if (!ALLOWED_EXTENSIONS.has(ext)) {
      throw new ApiError(400, `File extension ${ext} is not permitted. Only PDF, images, and DOCX documents are allowed.`);
    }
    ...
    if (!matched) {
      logger.warn(`SECURITY ALERT: Malicious file upload attempt! Extension was reported as '${ext}' but magic bytes did not match.`);
      throw new ApiError(400, 'Malicious upload attempt: File signature/magic bytes mismatch with reported file extension.');
    }

    // 3. Resolve secure folder target
    const folderInput = (req.body.folder || req.query.folder || 'syllabus') as string;
    ...
    const secureFilename = `${uuidv4().replace(/-/g, '')}${ext}`;
    const targetDir = path.resolve(env.UPLOAD_DIR, safeFolder);
