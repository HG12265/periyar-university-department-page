...
function resolveFontPath(): { name: string; path: string } {
  const baseDir = path.resolve(__dirname, '../..');
  const possibleFonts = [
    { name: 'ArialUnicode', path: path.join(baseDir, 'fonts', 'arialuni.ttf') },
    { name: 'NotoSansTamil', path: path.join(baseDir, 'fonts', 'NotoSansTamil-Regular.ttf') },
...
function createSectionHeader(title: string): any[] {
  return [
...
function cleanTextForPdf(val: string): string {
  if (!val) return '';
  // Convert basic HTML tags <b>/<strong> to markdown-like structures or just strip them
  return val.replace(/<[^>]+>/g, '').trim();
}

export const generateResumePdf = (resumeData: any, outputPath: string): Promise<void> => {
    ...
    try {
      ...
      if (qualifications.length > 0) {
        ...
        qualifications.forEach((q: any) => {
          ...
          if (q.year_upto) yrStr += ` - ${q.year_upto}`;
          qualTableBody.push([
      ...
      const webpageList: any[] = [];
      if (personal.profile_url) webpageList.push({ text: `🌐 Profile URL: ${personal.profile_url}`, style: 'bulletItem' });
      if (personal.email) webpageList.push({ text: `✉ Email: ${personal.email}`, style: 'bulletItem' });
      ...
      if (universityPositions.length > 0) {
        ...
        universityPositions.forEach((pos: any) => {
          let dur = `${pos.month_from || ''} ${pos.year_from || ''}`;
          const upto = `${pos.month_upto || ''} ${pos.year_upto || ''}`.trim();
          dur += upto ? ` - ${upto}` : ' - Present';
      ...
      if (researchProjects.length > 0) {
        const projectList = researchProjects.map((p: any) => ({
      ...
      if (patents.length > 0) {
        const patentList = patents.map((p: any) => ({
      ...
      if (foreignVisits.length > 0) {
        const fvList = foreignVisits.map((v: any) => ({
      ...
      if (eventsOrganized.length > 0) {
        const eoList = eventsOrganized.map((e: any) => ({
      ...
      if (papersPublished.length > 0) {
        const pubList = papersPublished.map((p: any) => {
      ...
      if (papersPresented.length > 0) {
        const presList = papersPresented.map((p: any) => {
          const authors = [p.Author_1, p.Author_2, p.Author_3].filter(Boolean).join(', ');
          return {
      ...
      if (lecturesDelivered.length > 0) {
        const lecList = lecturesDelivered.map((l: any) => ({
      ...
      if (copyrights.length > 0) {
        const copyList = copyrights.map((c: any) => ({
      ...
      if (phdProduced.length > 0) {
        const phdList = phdProduced.map((txt: string) => ({
      ...
      if (awardsReceived.length > 0) {
        const awardList = awardsReceived.map((a: any) => ({
      ...
      doc.write(outputPath)
        .then(() => {
          logger.info(`PDF Resume generated successfully at ${outputPath}`);
          resolve();
        })
        .catch((err: Error) => {
          logger.error(`PDF write failed: ${err.message}`);
          reject(err);
        });
    } catch (error) {
      logger.error(`PDF generation process threw an error: ${(error as Error).message}`);
      reject(error);
    }
  });
};
