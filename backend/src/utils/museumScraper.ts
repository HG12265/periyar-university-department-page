...
export class MuseumScraper {
  
  static async scrapeAndSave(): Promise<{ status: string; message: string }> {
    ...
    try {
      ...
      logger.error(`Museum scraping HTTP fetch failed: ${e.message}. Falling back to default pre-scraped data.`);
    }
    ...
    const [deptRows] = await primaryDB.query<RowDataPacket[]>(
    ...
    const [sectionRows] = await primaryDB.query<RowDataPacket[]>(
      'SELECT id FROM department_sections WHERE dept_id = ? AND category = ? LIMIT 1',
      [deptId, 'museum']
    );
    ...
    if (sectionRows.length === 0) {
      ...
      await primaryDB.query(
        'INSERT INTO department_sections (dept_id, section_title, category, content, order_index) VALUES (?, ?, ?, ?, 10)',
        [deptId, 'Museum Details', 'museum', contentJsonStr]
      );
    } else {
      // Update section content
      await primaryDB.query(
        'UPDATE department_sections SET content = ? WHERE dept_id = ? AND category = ?',
        [contentJsonStr, deptId, 'museum']
      );
      
      // Delete any duplicates
      await primaryDB.query(
        'DELETE FROM department_sections WHERE dept_id = ? AND category = ? AND id != ?',
        [deptId, 'museum', sectionRows[0].id]
      );
    }
